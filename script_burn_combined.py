...
def burn_subtitles_nvenc(input_file, output_file, subtitle_inbuilt=False, subtitle_file="", preferred_track=None):
    ...
    if ANIME:
        ffmpeg_cmd.extend(['-c:v', 'hevc_nvenc', '-pix_fmt', 'p010le', '-preset', 'p6', '-tune', 'hq', '-cq', '20', '-bf', '0'])
    else:
        ffmpeg_cmd.extend(['-c:v', 'h264_nvenc', '-preset', 'p5', '-cq', '20', '-profile:v', 'high', '-level', '4.1', '-pix_fmt', 'yuv420p', '-bf', '0'])
